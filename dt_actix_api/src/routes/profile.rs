...
pub async fn get_profile(
    ...
    if let Some((user_id, username, bio)) = user_data {
        ...
        let is_following =
            sqlx::query("SELECT 1 FROM followers WHERE user_id = $1 AND follower_id = $2")
                .bind(&user_id)
                .bind(&user_id)
                .fetch_optional(pool)
                .await?
...
pub async fn follow_profile(
    (form, following_data, pool): (
        web::Path<Profile>,
        web::Json<ProfileFollow>,
        web::Data<PgPool>,
    ),
) -> Result<HttpResponse, AppError> {
...
pub async fn unfollow_profile(
    ...
    .map_err(|_| {
        AppError::NotFound(
            serde_json::json!({"error": "Record not found for the provided username"}),
        )
    })?;
