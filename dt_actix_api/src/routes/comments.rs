...
pub async fn get_articles_comments(
    ...
    let article_id: Vec<Uuid> =
        sqlx::query_as::<_, (Uuid,)>("SELECT id FROM articles WHERE slug = $1")
            .bind(&params.slug)
            .fetch_all(pool)
            .await?
            .into_iter()
    ...
    match query.execute(pool).await {
        Ok(_) => {
            ...
            let select_query = sqlx::query(
                "SELECT * FROM comments WHERE article_id = $1 AND user_id = $2 AND body = $3",
            )
            .bind(&new_comment.article_id)
            .bind(&new_comment.user_id)
            .bind(&new_comment.body);
            ...
            match select_query.fetch_one(pool).await {
                Ok(inserted_comment) => {
                    ...
                    let comment_response =
                        get_comment_response(comment_id, Some(user_id), pool).await?;
...
pub async fn delete_articles_comments(
    ...
    let (comment_id, user_id): (Uuid, Uuid) =
        sqlx::query_as::<_, (Uuid, Uuid)>("SELECT id, user_id FROM comments WHERE id = $1")
    ...
    let query = sqlx::query("DELETE FROM comments WHERE id = $1").bind(&comment_id);
...
async fn get_comment_list_response(
    ...
    let futures = comments.iter().map(|comment| {
    ...
    let comment_list: Result<Vec<CommentResponseInner>, AppError> = results.into_iter().collect();

    match comment_list {
        Ok(comments) => Ok(CommentListResponse { comments }),
        Err(e) => Err(e),
    }
}
