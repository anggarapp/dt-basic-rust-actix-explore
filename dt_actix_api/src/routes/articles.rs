...
pub async fn get_articles(
    ...
    let matched_articles: Vec<Article> = sqlx::query_as(&query).fetch_all(pool).await?;
    ...
    match response {
        Ok(article_list_response) => Ok(HttpResponse::Ok().json(article_list_response)),
        Err(e) => Err(e),
    }
}
...
pub async fn create_article(
    (form, username, pool): (
        web::Json<CreateArticle>,
        web::Path<UserForArticle>,
        web::Data<PgPool>,
    ),
) -> Result<HttpResponse, AppError> {
    ...
    match query.execute(pool).await {
        Ok(_) => {
            ...
            let article_response =
                get_article_response(new_article.slug, Some(new_article.author_id), pool).await?;
...
pub async fn get_articles_feed(
    (params, username, pool): (
        web::Query<FeedParams>,
        web::Path<UserForArticle>,
        web::Data<PgPool>,
    ),
) -> Result<HttpResponse, AppError> {
    ...
    let following_ids: Vec<Uuid> =
        sqlx::query_as::<_, (Uuid,)>("SELECT user_id FROM followers WHERE follower_id = $1")
            .bind(&user_id)
            .fetch_all(pool)
            .await?
            .into_iter()
...
pub async fn update_articles_by_slug(
    (path, username, form, pool): (
        web::Path<ArticlePath>,
        web::Query<UserForArticle>,
        web::Json<UpdateArticleOuter>,
        web::Data<PgPool>,
    ),
) -> Result<HttpResponse, AppError> {
    ...
    let (article_id, article_author_id, article_slug): (Uuid, Uuid, String) =
        sqlx::query_as::<_, (Uuid, Uuid, String)>(
            "SELECT id, author_id, slug FROM articles WHERE slug = $1",
        )
        .bind(&path.slug)
        .fetch_one(pool)
        .await
    ...
    match article.fetch_one(pool).await {
        Ok(res) => {
            ...
            let article_response: ArticleResponse =
                get_article_response(article_slug, Some(record_author_id), pool).await?;
...
pub async fn delete_articles_by_slug(
    ...
    let (article_id, article_author_id): (Uuid, Uuid) = sqlx::query_as::<_, (Uuid, Uuid)>(
        "SELECT id, author_id, slug FROM articles WHERE slug = $1",
    )
    .bind(&path.slug)
    .fetch_one(pool)
    ...
    let query = sqlx::query("DELETE FROM articles WHERE id = $1").bind(&article_id);
...
pub async fn favorite_articles_by_slug(
    ...
    let (article_id, article_slug): (Uuid, String) =
        sqlx::query_as::<_, (Uuid, String)>("SELECT id, slug FROM articles WHERE slug = $1")
            .bind(&path.slug)
            .fetch_one(pool)
            .await
            .map_err(|_| AppError::InternalServerError)?;
...
async fn get_article_list_response(
    ...
    let articles: Result<Vec<ArticleResponseInner>, AppError> = results.into_iter().collect();

    match articles {
        Ok(article_list) => Ok(ArticleListResponse {
...
async fn get_article_response(
    ...
    let (favorited, following) = match user_id {
        Some(user_id) => {
            get_favorited_and_following(data.article.id, data.author.id, user_id, pool).await?
        }
        None => (false, false),
    };
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let tag_results: Result<Vec<ArticleTag>, AppError> =
        Ok(try_join_all(tags.into_iter().map(|tag_name| {
