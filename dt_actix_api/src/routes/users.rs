...
use crate::schemas::*;
use crate::utils::{hash_password, validation_errors_response};
...
pub async fn login(
    ...
    let query = sqlx::query("SELECT password FROM users WHERE email = $1").bind(&login_user.email);
...
pub async fn update(
    ...
    let query =
        sqlx::query("UPDATE users SET email = $1, password = $2, bio = $3 WHERE username = $4")
...
pub async fn delete(
    ...
    let query = sqlx::query("DELETE FROM users WHERE username = $1").bind(&user_info.username);
