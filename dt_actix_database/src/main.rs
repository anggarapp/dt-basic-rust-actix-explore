...
async fn get_item(pool: web::Data<DbPool>, item_id: web::Path<Uuid>) -> impl Responder {
    let mut conn = pool.get().expect("Couldn't get DB connection from pool");
...
async fn delete_item(pool: web::Data<DbPool>, item_id: web::Path<Uuid>) -> impl Responder {
    let mut conn = pool.get().expect("Couldn't get DB connection from pool");
